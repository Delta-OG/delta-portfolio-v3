...
const CanvasWrapper = ({ children }: { children: React.ReactNode }) => {
  ...
  return (
    <ErrorBoundary fallback={<div className="w-full h-full absolute inset-0 bg-black" />}>
      <Suspense fallback={<div className="w-full h-full absolute inset-0 bg-black" />}>
        <Canvas
          dpr={[1, 2]}
          frameloop="always"
...
const MergedPlanes = forwardRef<any, MergedPlanesProps>(({ material, width, count, height }, ref) => {
  ...
  useFrame((_, delta) => {
    if (mesh.current && material && "uniforms" in material) {
      try {
        material.uniforms.time.value += 0.1 * delta
      } catch (error) {
        console.error("Error updating material time:", error)
      }
    }
  })
...
const PlaneNoise = forwardRef<any, PlaneNoiseProps>((props, ref) => (
